...
WHEEL_DIAMETER_MM = 65.0    # outer diameter of one wheel (mm)
TRACK_WIDTH_MM    = 245.0   # centre-to-centre distance between left and right wheels (mm)
...
DUTY_CYCLE_MIN   = 2.9   # % — sensor output at the 0 deg origin
DUTY_CYCLE_MAX   = 97.1  # % — sensor output just before one full CW revolution
UNITS_PER_CIRCLE = 360   # work in degrees
 
PULSE_MIN_US = 1000      # us — full clockwise
PULSE_MAX_US = 2000      # us — full counter-clockwise
...
class FeedbackReader:
    ...
    def _on_edge(self, gpio, level, tick):
        ...
        else:           # falling edge — end of high pulse
    ...
    def cleanup(self):
        self._cb.cancel()   # cancel the pigpio edge callback — nothing else
...
class Servo360:
    ...
    def stop(self):
        # Send explicit 1500 us neutral pulse — the Parallax 360 stops
        # reliably on this value regardless of prior throttle setting.
        self._servo.throttle = 0.2
        time.sleep(0.02)  # hold for at least one 50 Hz PWM cycle
...
class RoboticDog:
    ...
    def __init__(self):
        ...
        for name, (channel, inverted, odo_inverted) in SERVO_CHANNELS.items():
            ...
            self.servos[name] = Servo360(
                ...
                inverted=inverted, odo_inverted=odo_inverted,
            )
            print(f"  {name:>12s}  channel={channel}  "
                  f"GPIO={pin}  inverted={inverted}  odo_inverted={odo_inverted}")
    ...
    def cleanup(self):
        self.stop_all()
        time.sleep(0.1)   # give PCA9685 time to latch the stop pulse
        for s in self.servos.values():
            s.cleanup()
        self._pi.stop()
